...
export function CRMKanbanView({ leads, onLeadClick, onLeadUpdated }: CRMKanbanViewProps) {
  ...
  const [selectedLeadForMove, setSelectedLeadForMove] = useState<CRMLead | null>(null);
  ...
  const handleMobileMove = async (newStage: string) => {
    if (!selectedLeadForMove || selectedLeadForMove.stage === newStage) {
      setSelectedLeadForMove(null);
      return;
    }

    try {
      await updateLead(selectedLeadForMove.id, { stage: newStage });
      toast.success(`Moved to ${LEAD_STAGES.find(s => s.value === newStage)?.label}`);
      onLeadUpdated();
    } catch (error) {
      toast.error('Failed to update lead');
  ...
  return (
    <>
      <div className="flex gap-2 md:gap-3 overflow-x-auto pb-4 -mx-3 px-3 md:mx-0 md:px-0 snap-x snap-mandatory md:snap-none">
        {LEAD_STAGES.map((stage) => {
          ...
          return (
            <div
              key={stage.value}
              className={`flex-shrink-0 w-[75vw] sm:w-64 md:w-72 flex flex-col rounded-lg border-2 transition-colors snap-center ${
            ...
            >
              <div className={`p-2 md:p-3 border-b ${getStageColor(stage.value)}`}>
                <div className="flex items-center justify-between">
                  <h3 className={`font-semibold text-sm md:text-base ${getStageHeaderColor(stage.value)}`}>
                    {stage.label}
                  </h3>
              ...
              <div className="flex-1 p-1.5 md:p-2 space-y-1.5 md:space-y-2 min-h-[180px] md:min-h-[200px] max-h-[calc(100vh-400px)] overflow-y-auto">
                {stageLeads.length === 0 ? (
                    ...
                    >
                      <CardContent className="p-2 md:p-3">
                        <div className="flex items-start justify-between gap-1">
                          <h4 className="font-medium text-xs md:text-sm truncate flex-1">
                            {lead.name}
                          </h4>
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-6 w-6 p-0 md:hidden shrink-0"
                            onClick={(e) => {
                              e.stopPropagation();
                        ...
                        <div className="space-y-0.5 md:space-y-1 text-[10px] md:text-xs text-muted-foreground mt-1">
                          {lead.phone && (
      ...
      <Dialog open={!!selectedLeadForMove} onOpenChange={() => setSelectedLeadForMove(null)}>
        <DialogContent className="max-w-[90vw] sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-base">Move to Stage</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-1 gap-2">
            {LEAD_STAGES.map((stage) => (
              <Button
                key={stage.value}
                variant={selectedLeadForMove?.stage === stage.value ? 'secondary' : 'outline'}
                className={`justify-start ${getStageHeaderColor(stage.value)}`}
                disabled={selectedLeadForMove?.stage === stage.value}
                onClick={() => handleMobileMove(stage.value)}
              >
                {stage.label}
                {selectedLeadForMove?.stage === stage.value && (
