        ...
        <div className="flex items-center justify-between h-16 sm:h-20 md:h-28">
          ...
          <nav className="hidden md:flex items-center space-x-6 lg:space-x-8">
            ...
            <div className="relative" onMouseEnter={() => setIsResidentialOpen(true)} onMouseLeave={() => setIsResidentialOpen(false)}>
              <button className="text-gray-600 hover:text-blue-600 transition-colors duration-200 font-medium text-xs lg:text-sm flex items-center">
                Residential
                <ChevronDown className="ml-1 h-3 w-3" />
              </button>
              {isResidentialOpen && <div className="absolute top-full left-0 mt-0 w-48 bg-white rounded-lg shadow-xl border border-gray-200 py-2 z-50">
                  {residentialItems.map(item => <a key={item.name} href={item.path} className="block px-4 py-2 text-sm text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-colors">
                      {item.name}
                    </a>)}
                </div>}
            </div>
            ...
            <div className="relative" onMouseEnter={() => setIsCommercialOpen(true)} onMouseLeave={() => setIsCommercialOpen(false)}>
              <button onClick={() => navigate('/commercial')} className="text-gray-600 hover:text-blue-600 transition-colors duration-200 font-medium text-xs lg:text-sm flex items-center">
                Commercial
                <ChevronDown className="ml-1 h-3 w-3" />
              </button>
            ...
            <div className="relative" onMouseEnter={() => setIsAboutOpen(true)} onMouseLeave={() => setIsAboutOpen(false)}>
              ...
              {isAboutOpen && <div className="absolute top-full left-0 mt-0 w-48 bg-white rounded-lg shadow-xl border border-gray-200 py-2 z-50">
                  {aboutItems.map(item => <a key={item.name} href={item.path} className="block px-4 py-2 text-sm text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-colors">
                      {item.name}
                    </a>)}
                </div>}
            </div>
            
            {navItems.slice(1).map(item => <a key={item.name} href={item.path} className="text-gray-600 hover:text-blue-600 transition-colors duration-200 font-medium text-xs lg:text-sm">
                {item.name}
              </a>)}
        ...
        {isMenuOpen && <div className="md:hidden border-t border-gray-100 py-4 bg-white">
            <nav className="flex flex-col space-y-4">
              ...
              <div className="px-4">
                ...
                {residentialItems.map(item => <a key={item.name} href={item.path} className="block text-gray-600 hover:text-blue-600 transition-colors duration-200 font-medium py-2 text-sm pl-4 rounded-lg hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>
                    {item.name}
                  </a>)}
              </div>
              ...
              <div className="px-4">
                ...
                {aboutItems.map(item => <a key={item.name} href={item.path} className="block text-gray-600 hover:text-blue-600 transition-colors duration-200 font-medium py-2 text-sm pl-4 rounded-lg hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>
                    {item.name}
                  </a>)}
              </div>
