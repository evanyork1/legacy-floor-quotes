...
const Quote = () => {
  ...
  const colorOptions = [{
    ...
    id: "cabin-fever",
    name: "Cabin Fever",
    thumbnail: "/lovable-uploads/735e5c66-d9b6-4902-9d1a-fd1deea7b369.png",
    preview: "/lovable-uploads/e6d46c38-cab4-4c0e-b5f7-a13f414dc01b.png"
  }, {
    id: "coyote",
    name: "Coyote",
  ...
  const calculatePrice = () => {
    ...
    // Main space
    if (formData.garageType === "2-car") totalSqft += 425;else if (formData.garageType === "3-car") totalSqft += 650;else if (formData.garageType === "4-car") totalSqft += 900;else if (formData.garageType === "custom" && formData.customSqft) totalSqft += parseInt(formData.customSqft);

    // Additional spaces
    formData.additionalSpaces.forEach(space => {
      if (space.type === "2-car") totalSqft += 425;else if (space.type === "3-car") totalSqft += 650;else if (space.type === "4-car") totalSqft += 900;else if (space.type === "custom" && space.sqft) totalSqft += parseInt(space.sqft);
    });
        ...
        return <div className="space-y-6 sm:space-y-8">
            ...
            <div className="mt-8 sm:mt-12 max-w-2xl mx-auto px-4">
              ...
              {showAdditionalSpace && <div className="mt-4 sm:mt-6 p-4 sm:p-6 bg-purple-50 rounded-xl border border-purple-200">
                  ...
                  id: "other",
                  label: "Other"
                }].map(option => <button key={option.id} onClick={() => setCurrentAdditionalSpace(prev => ({
                  ...prev,
                  type: option.id
                }))} className={`p-3 sm:p-4 rounded-lg border-2 text-left transition-all ${currentAdditionalSpace.type === option.id ? 'border-purple-600 bg-purple-100' : 'border-gray-200 hover:border-purple-300'}`}>
                  ...
                  {currentAdditionalSpace.type === "other" && <div className="mt-3 sm:mt-4">
                      <Input value={currentAdditionalSpace.otherSpaceType} onChange={e => setCurrentAdditionalSpace(prev => ({
                  ...prev,
                  otherSpaceType: e.target.value
                }))} placeholder="Please describe your space" className="h-10 sm:h-12" />
                    </div>}

                  {currentAdditionalSpace.type === "custom" && <div className="mt-4 sm:mt-6 space-y-3">
                      <Input type="number" value={currentAdditionalSpace.sqft} onChange={e => setCurrentAdditionalSpace(prev => ({
                  ...prev,
                  sqft: e.target.value
                }))} placeholder="Enter square footage" className="h-12" />
                    ...
                    ...prev,
                    spaceType: option.id
                  }))} className={`p-3 rounded-lg border text-left transition-all ${currentAdditionalSpace.spaceType === option.id ? 'border-purple-600 bg-purple-100' : 'border-gray-200 hover:border-purple-300'}`}>
                            {option.label}
                          </button>)}
                      </div>
                      {currentAdditionalSpace.spaceType === "other" && <Input value={currentAdditionalSpace.otherSpaceType} onChange={e => setCurrentAdditionalSpace(prev => ({
                  ...prev,
                  otherSpaceType: e.target.value
                }))} placeholder="Please describe your space" className="h-12" />}
                    </div>}
            ...
            <div className="max-w-xl mx-auto space-y-4 sm:space-y-6 px-4">
              ...
              <div>
                ...
                <div className="grid gap-2 sm:gap-3">
                  ...
                  id: "other",
                  label: "Other"
                }].map(option => <button key={option.id} onClick={() => updateFormData('spaceType', option.id)} className={`p-3 sm:p-4 rounded-lg border-2 text-left transition-all ${formData.spaceType === option.id ? 'border-blue-600 bg-blue-50' : 'border-gray-200 hover:border-blue-300'}`}>
                      <div className="flex items-center justify-between">
            ...
            <div className="max-w-2xl mx-auto px-4">
              ...
              <div className="mb-6 sm:mb-8">
                ...
                <div className="bg-gray-100 border-2 border-dashed border-gray-300 rounded-xl h-48 sm:h-96 flex items-center justify-center overflow-hidden">
                  <img src="/lovable-uploads/c131885b-6fd6-4475-afeb-d13b0d895942.png" alt="Example garage exterior view" className="w-full h-full object-cover rounded-xl" />
                </div>
              </div>
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 1:
        return formData.garageType !== "";
      case 2:
        return formData.garageType !== "custom" || formData.customSqft !== "" && formData.spaceType !== "";
      case 3:
        return true;
      // Photos are optional
      case 4:
        return true;
      // Damage photos are optional
      case 5:
        return formData.colorChoice !== "";
      case 6:
        return formData.name !== "" && formData.email !== "" && formData.phone !== "" && formData.zipCode !== "";
      case 7:
        return true;
      default:
        return false;
          ...
          <Card className="shadow-2xl border-0 overflow-hidden">
            <CardContent className="p-6 sm:p-8 lg:p-12">
              ...
              <div className="flex flex-col sm:flex-row justify-between gap-3 sm:gap-0 mt-8 sm:mt-12 pt-6 sm:pt-8 border-t border-gray-100">
                ...
                {currentStep < totalSteps ? <Button onClick={nextStep} disabled={!canProceed()} className="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 flex items-center justify-center px-6 sm:px-8 py-2 sm:py-3 text-base sm:text-lg order-1 sm:order-2">
                    Next Step
                    <ArrowRight className="h-4 w-4 sm:h-5 sm:w-5 ml-2" />
                  </Button> : <Button onClick={handleSubmit} className="bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 flex items-center justify-center px-6 sm:px-8 py-2 sm:py-3 text-base sm:text-lg order-1 sm:order-2">
                    Finish & Submit
